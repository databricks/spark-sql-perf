...
class DBGEN(dbgenDir: String, skew: Boolean = false) extends DataGenerator {
  ...
  def generate(sparkContext: SparkContext,name: String, partitions: Int, scaleFactor: String) = {
    ...
    val generatedData = {
      sparkContext.parallelize(1 to numPartitions, numPartitions).flatMap { i =>
        ...
        val skewParam = if (skew) "-k" else ""
        val commands = Seq(
          "bash", "-c",
          s"cd $localToolsDir && ./dbgen -q $skewParam -T ${shortTableNames(name)} -s $scaleFactor $parallel")
        println(commands)
        commands.lines
...
class TPCHTables(
  ...
  val dataGenerator = new DBGEN(dbgenDir, skew)
